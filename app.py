...
def generate_prompt():
    client_file_path = f"./templates/clients/{client}.txt"
    ...
    if client_file_content == None or procedure_file_content == None:
        ...
        if phase == 'All':
            ...
            prompt = f"1.Provide only the {phase} phase.\n\n"\
...
st.set_page_config(page_title="PF Proposal Writer", page_icon=":shark:", layout="centered")

#Website Structure
with st.sidebar:
    st.title("Project Fuel LLC")
    selected_function = st.selectbox("Function:", ['Proposal Template Designer', 'AI Proposal Development'])
    breq = st.text_input("Business Requirement:")
    col1,col2,col3 = st.columns(3)
    with col1:
        file_paths = os.listdir("./templates/clients")
        clients_list = [os.path.splitext(os.path.basename(path))[0] for path in file_paths]
    ...
    with col3:
        products = st.selectbox("Products:", ['No', 'Yes'], key='products')
    ...
    if selected_function == 'AI Proposal Development':
        ...
        st.selectbox("Proposal Templates:",['test'])
...
if selected_function == 'AI Proposal Development':
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            ...
            st.download_button(label='Download CSV', data=message['content'], file_name=breq+".csv", mime='text/csv')
else:
    st.markdown("<p style='text-align: center; color: black;'>Project Fuel Template Designer</p>", unsafe_allow_html=True)
    proposal_name = 'Proposal Name'
    st.header(proposal_name, divider="grey")
    st.write("Business requirement: " + breq)
    client_file_path = f"./templates/clients/{client}.txt"
    procedure_file_path = f"./templates/{procedure}/{technology}.txt"
    client_file_content = read_text_file(client_file_path)
    procedure_file_content = read_text_file(procedure_file_path)
    ...
    if client_file_content == None or procedure_file_content == None:
        ...
        if phase == 'All':
            content = f"1.Create an IT workplan using the following business requirement:\n\n{breq}.\n\n"\
                    f"2.Change the existing and new firewalls multiplier quantity from the business requirement\n\n"\
                    f"3.Calculate the Total multiplying Hours and Mutiplier.\n\n"\
                    f"4.Present content as table and provide the output without any additional comments.\n\n"\
                    f"5.{client_file_content}.\n\n"\
            ...
            content = f"1.Provide only the {phase} phase.\n\n"\
                    f"2.Create an IT workplan using the following business requirement:\n\n{breq}.\n\n"\
                    f"3.Change the existing and new firewalls multiplier quantity from the business requirement.\n\n"\
                    f"4.Calculate the Total multiplying Hours and Mutiplier.\n\n"\
                    f"5.Present content as table and provide the output without any additional comments.\n\n"\
